...
function loanSchedule(principal: number, annualRate: number, termMonths: number) {
  const r = annualRate / 100 / 12;
  const n = termMonths;
  const pmt = annualRate === 0 ? principal / n
...
function LoanCalc() {
  ...
  const schedule = useMemo(() => (showSchedule && result) ? loanSchedule(p, r, n) : [], [p, r, n, showSchedule, result]);
      ...
      <Card shadow="sm">
        <CardBody className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            >
              {TERMS.map((m) => (
                <SelectItem key={String(m)} textValue={`${m} mo`}>{m} {t("finCalc.months")}</SelectItem>
              ))}
            </Select>
          </div>
        </CardBody>
...
function BreakevenCalc() {
  ...
  const result = useMemo(() => (f > 0 && p > 0) ? breakevenCalc(f, v, p) : null, [f, v, p]);
...
function ROICalc() {
  ...
  return (
    <div className="space-y-4">
      <Card shadow="sm">
        <CardBody className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <NumInput label={t("finCalc.investment")} value={investment} onChange={setInvestment} />
          <NumInput label={t("finCalc.netReturn")}  value={netReturn}  onChange={setNetReturn} />
          <NumInput label={t("finCalc.timePeriod")} value={years}      onChange={setYears} placeholder={t("finCalc.optional")} />
        </CardBody>
      </Card>
